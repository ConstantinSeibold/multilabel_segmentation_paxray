...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, residual = True):
...
def choose_resnet( name='resnet101', pretrained=False, input_nc=3, classes=1000,resume=None):
    print('The chosen Model was a {}'.format(name))
    if name.startswith('resnet'):
        resnet =  StandardResNet(name, pretrained, input_nc, classes)
    return resnet


class StandardResNet(nn.Module):
    def __init__(self, name='resnet50', pretrained=False, input_nc=3, classes=1000):
        super(StandardResNet, self).__init__()
        if name == 'resnet101':
            self.network = resnet101(pretrained)
        elif name == 'resnet152':
            self.network = resnet152(pretrained)
        elif name == 'resnet50':
        ...
        if input_nc!=3:
            self.network.conv1 = torch.nn.Conv2d(input_nc,64,(7,7),stride=(2,2),padding=(3,3),bias=False)
        if classes != 1000:
            self.network.fc = torch.nn.Linear(2048, classes)
    ...
    def forward(self, x, insert_layer = None, return_layer = []):
        ...
        if  5 in return_layer:
            result['feats_{}_map'.format(5)] = x
            if 5 == max(return_layer):
                return result

        if len(return_layer) != 0 and max(return_layer) <= 5:
            return result

        x_shape = x.shape
        result['visualization_cam'] = self.network.fc(x.flatten(-2).permute(0,2,1)).permute(0,2,1).view(x_shape[0],-1,x_shape[-2], x_shape[-1])

        x = self.network.avgpool(x)

        x = torch.flatten(x, 1)
        result['feats_{}_pooled'.format(5)]  = x

        x = self.network.fc(x)
        self.score = x.clone().detach()
        result['class_logits'] = x
