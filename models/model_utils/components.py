...
class UpInit(nn.Module):
    ...
    def __init__(self, temp_channels, in_channels, out_channels, bilinear=True):
        super().__init__()
...
class InputTransition(nn.Module):
    def __init__(self, inch,  outChans, elu):
        super(InputTransition, self).__init__()
        self.conv1 = nn.Conv2d(inch, outChans, kernel_size=5, padding=2)
...
class OutputTransition(nn.Module):
    def __init__(self, inChans, outChans, elu, nll):
        ...
        self.conv1 = nn.Conv2d(inChans, inChans, kernel_size=5, padding=2)
        self.bn1 = ContBatchNorm2d(inChans)
...
class ASPPConv(nn.Sequential):
    def __init__(self, in_channels, out_channels, dilation, norm='batch'):
        modules = [
...
class ASPP(nn.Module):
    def __init__(self, in_channels, atrous_rates, norm='batch'):
        ...
        modules.append(ASPPConv(in_channels, out_channels, rate1, norm = norm))
        modules.append(ASPPConv(in_channels, out_channels, rate2, norm = norm))
        modules.append(ASPPConv(in_channels, out_channels, rate3, norm = norm))
        modules.append(ASPPPooling(in_channels, out_channels, norm = norm))
